...
TEST(UniformRandomCFP, evenSpread) {
  /*
  ...
  std::vector<UniformRandomCFP*> tps(TPS);
  for (u32 idx = 0; idx < TPS; idx++) {
    tps.at(idx) = new UniformRandomCFP(
...
TEST(UniformRandomCFP, noSelf) {
  ...
  UniformRandomCFP tp("TP", nullptr, TOTAL, ME, settings);
