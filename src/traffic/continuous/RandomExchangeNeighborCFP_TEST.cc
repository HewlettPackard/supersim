 ...
 */
#include "traffic/continuous/RandomExchangeNeighborCFP.h"
...
TEST(RandomExchangeNeighborCFP, evenSpread_1d) {
  ...
  std::vector<RandomExchangeNeighborCFP*> tps(numTerminals);
  for (u32 idx = 0; idx < numTerminals; idx++) {
    tps.at(idx) = new RandomExchangeNeighborCFP(
...
TEST(RandomExchangeNeighborCFP, evenSpread_1d_all_terminals) {
  TestSetup test(1, 1, 0xBAADF00D);
  Json::Value settings;
...
TEST(RandomExchangeNeighborCFP, evenSpread_2d_all_terminals) {
  TestSetup test(1, 1, 0xBAADF00D);
  Json::Value settings;
