...
TEST(RandomDFP, evenSpread) {
  ...
  std::vector<RandomDFP*> tps(TPS);
  for (u32 idx = 0; idx < TPS; idx++) {
    tps.at(idx) = new RandomDFP(
...
TEST(RandomDFP, noSelf) {
  ...
  RandomDFP tp("TP", nullptr, TOTAL, ME, settings);
